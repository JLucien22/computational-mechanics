...
```{code-cell} ipython3
def sincos(x):
    '''Compute the sine and cosine of the input array'''
    sinx=np.sin(x)
    cosx=np.cos(x)
    return sinx,\
           cosx
print('A)', sincos([1,2,3,]))
array=np.arange(0,(2*np.pi),np.pi/12)
sin_x,cos_x=sincos(array)
plt.plot(sin_x,color='red', linestyle='-', label='sine')
plt.plot(cos_x,color='blue', linestyle='-', label='cosine')
```
...
for i in range(10):
    for j in range(10):
        A_99.append(A1[i]*A2[j])
        Sum+=A1[i]*A2[j]
        total_terms=len(A_99)
        mean=Sum/total_terms
        stand1+=((A1[i]*A2[j])-mean)**2
stand2=stand1/total_terms
std=np.sqrt(stand2)

print('B) The mean is',mean)
print('C) The standard deviation is', std)
